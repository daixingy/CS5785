...
def predict_one_vs_all(all_theta, X):
    ...
    rs = np.dot(all_theta, X.T)
    rs = np.roll(rs, -1, axis=0)
    rs = np.vstack([np.zeros(m), rs])
    p = np.argmax(rs, axis=0)
    
    rs = sigmoid(np.dot(X, all_theta.T))
    rs = np.roll(rs, -1, axis=1)
    rs = np.hstack([np.zeros(m)[:, np.newaxis], rs])
    p = np.argmax(rs, axis=1)
...
if __name__ == "__main__":
    ...
    yhat = predict_one_vs_all(theta, test)
    print(yhat)
    print(yhat - test_label)
    result = (yhat - test_label).tolist()
    count = 0
    ...
    # print(yhat - validation_label)
    # print(theta)
    # model = LogisticRegression()
    # ovr = OneVsRestClassifier(model)
    # ovr.fit(train, train_label)
    # # make predictions
    # yhat = ovr.predict(validation)
    # print(yhat)
    # print(validation_label)
    # print(yhat - validation_label)
    # yhat = ovr.predict(test)
    # print(yhat)
    # print(test_label)
    # print(yhat - test_label)